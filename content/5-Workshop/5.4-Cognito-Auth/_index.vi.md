...
Trong phần này, tiến hành tạo và cấu hình một Amazon Cognito User Pool nhằm phục vụ cho hoạt động đăng ký, đăng nhập tài khoản người dùng, đồng thời tích hợp bảo vệ API Gateway.

Tùy thuộc vào trạng thái tài khoản và AWS Region của người thực hiện, giao diện điều khiển Console có thể hiển thị dưới dạng **Trình hướng dẫn 6 bước chuẩn (Standard Wizard)** hoặc **Trình hướng dẫn thiết lập nhanh rút gọn (Simplified Wizard)**. Hãy chọn luồng cấu hình tương ứng dưới đây.
...
Nếu nhìn thấy giao diện **"Set up resources for your application"** giống như trong screenshot:
...
3. **Add a return URL - optional:**
   * Để trống (sử dụng các biểu mẫu đăng nhập tự tùy biến ở React frontend chứ không sử dụng giao diện đăng nhập mặc định Hosted UI của Cognito).
...
Thiết lập thêm thuộc tính tùy biến `custom:role` để kiểm soát xem người dùng là user thường hay admin.
1. Trong giao diện Cognito Console, bấm chọn User Pool vừa được tạo ở trên.
...
Định nghĩa hai nhóm quản trị:
1. Trong User Pool, chuyển sang tab **User groups**.
2. Chọn **Create group**.
...
*Lưu ý: Hãy ghi lại hai thông số **User Pool ID** và **App Client ID** hiển thị ở trang thông tin tổng quan của User Pool. sẽ cần chúng để cấu hình API Authorizer và React frontend sau này.*
