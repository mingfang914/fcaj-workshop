...
Trước khi bắt đầu cấu hình hệ thống **Smart Image Platform**, hãy đảm bảo đã chuẩn bị đầy đủ các yếu tố sau:

### 1. Tài khoản AWS (AWS Account)
* cần có một tài khoản **AWS Account** đang hoạt động.
...
Cài đặt các công cụ sau trên máy cá nhân nếu muốn build hoặc deploy mã nguồn bằng lệnh:
...
* Đẩy mã nguồn dự án (`AWS-Project`) lên một **GitHub Repository** (ở chế độ public hoặc private).
* Chuẩn bị sẵn một **GitHub Personal Access Token (PAT)** có quyền `repo` và `admin:repo_hook` hoặc thực hiện liên kết xác thực tài khoản GitHub với dịch vụ AWS Amplify trong quá trình thiết lập.
