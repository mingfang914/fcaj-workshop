...
Trong phần này, tiến hành tạo các IAM Policy và Role trước, sau đó triển khai mã nguồn lên Lambda, thiết lập các bộ kích hoạt tự động và cấu hình Amazon API Gateway.

---

### Bước 1: Tạo các chính sách IAM tùy chỉnh (Custom IAM Policies)

Trước khi tạo các vai trò thực thi (execution roles) cho Lambda, cần tạo các chính sách phân quyền (permission policies) tùy chỉnh trước trong IAM.
...
Sau khi đã tạo xong các chính sách phân quyền ở Bước 1, tiến hành tạo các vai trò thực thi và gán chính sách tương ứng vào.
...
5. **Cấu hình Cognito Authorizer:**
   * **Lưu ý:** Để nhìn thấy menu điều hướng riêng của API (trong đó có mục "Authorizers"), trước tiên cần **bấm chọn tên API của người thực hiện** (ví dụ `SmartImage-API-production` hoặc `SmartImage-API-staging`) từ danh sách API hiển thị ở trang chủ.
 ...
 6. **Cấu hình Resource & Method Proxy:**
   ...
   * Bấm chọn thư mục gốc `/` (hoặc `/v1` nếu muốn triển khai dưới đường dẫn `/v1`) trong cây thư mục Resources.
...
![Cấu hình API Gateway Authorizer](/images/5-Workshop/5.5-Backend-Serverless/api_gateway_authorizer.png)
   ...
   * *Hãy ghi lại **Invoke URL** của API Gateway (ví dụ: `https://xxxx.execute-api.ap-southeast-1.amazonaws.com/dev`). sẽ dùng nó để cấu hình React frontend.*
