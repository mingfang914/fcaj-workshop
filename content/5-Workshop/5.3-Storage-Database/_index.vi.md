...
Trong phần này, tiến hành tạo các lớp lưu trữ (Amazon S3 buckets) và cơ sở dữ liệu (Amazon DynamoDB) cần thiết cho Smart Image Platform.
...
Tiến hành khởi tạo ba bảng DynamoDB: `SmartImage-Images` (bảng lưu trữ metadata chính), `SmartImage-UserQuotas` (bảng quản lý hạn mức người dùng), và `SmartImage-UserProfiles` (bảng thông tin hồ sơ).
...
6. **Attribute projections:**
   * **Cách 1: Khuyên dùng (Giống hệt cấu hình CDK):** Tích chọn **Include**. Do bảng mới tạo chưa có dữ liệu, người thực hiện cần tự gõ thủ công từng tên thuộc tính dưới đây và bấm **Add** sau mỗi từ:
