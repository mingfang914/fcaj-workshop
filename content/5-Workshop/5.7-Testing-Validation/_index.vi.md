...
4. Kiểm tra hộp thư email. sẽ nhận được một email tự động từ Cognito chứa mã xác nhận kích hoạt tài khoản.
...
3. Kiểm tra danh sách tệp. sẽ thấy một folder mới được tạo chứa ảnh thumbnail của tệp ảnh người thực hiện vừa upload (ảnh đã được thu nhỏ kích thước và nén dung lượng, thường có hậu tố `-thumbnail`).
...
3. Tìm bản ghi tương ứng với ID tài khoản và tệp ảnh vừa upload.
...
3. sẽ thấy hình ảnh vừa upload hiển thị trên giao diện gallery.
...
Nếu đã triển khai monitoring stack:
...
3. Kiểm tra các biểu đồ đồ thị (**Graph Widgets**) để đo lường các thông số:
   * **Invocations:** Lượng request gọi Lambda tương ứng với số ảnh đã upload.
...
Giả lập tình huống lỗi để kiểm tra khả năng bắt lỗi và đưa thông tin vào hàng đợi Dead Letter Queue (DLQ):
...
2. **Kiểm tra Logs lỗi của Image Processor:**
   ...
   * Mở stream log mới nhất vừa sinh ra. sẽ thấy dòng log ghi nhận lỗi xử lý định dạng tệp: e.g. `Error: Invalid file format` hoặc `Sharp: Input buffer has an unsupported image format`.
3. **Đo lường sai số trên Dashboard & Alarms:**
   ...
   * Hệ thống sẽ tự động gửi một email cảnh báo đến địa chỉ email quản trị viên của người thực hiện thông qua SNS.
4. **Kiểm tra hàng đợi Dead Letter Queue (DLQ):**
   ...
   * sẽ thấy số lượng tin nhắn chờ duyệt (`Messages Available`) tăng lên đại diện cho request lỗi vừa bị đẩy từ Lambda sang để quản trị viên kiểm tra.
